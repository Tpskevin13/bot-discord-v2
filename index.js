...
async function handleCommand(command, args, msg, inter) {
    ...
    if (command === 'beg') {
        ...
        return reply(null, E(r.win?'🙏 ¡Conseguiste algo!':'🙏 Nadie te dio nada...',r.msg,r.win?'#8BC34A':'#9E9E9E').addField(r.win?'💰 Conseguiste':'😔 Ganaste',r.win?`${r.amount} 🪙`:'0 🪙',true).addField('👛 Total',`${r.total} 🪙`,true).setFooter({text:'Cooldown: 5 minutos'}));
    }
    ...
    if (command === 'spin') {
        ...
        const desc=`La bolita cayó en el **${r.n}** — ${r.color}\n\nApostaste: **${r.bet} 🪙**\n${r.earned>0?`+**${r.earned} coins** 🎉`:`💸 Perdiste **${r.bet} coins**`}`;
        return reply(null, E('🎰 Ruleta',desc,r.earned>0?'#FFD700':'#E53935').addField('👛 Total',`${r.total} 🪙`,true).setFooter({text:'Verde (0) paga 35x | Rojo/Negro pagan 1x | Cooldown: 30 min'}));
    }
