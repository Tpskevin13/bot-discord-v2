...
client.on('ready', async () => {
    ...
    if (guild) {
        ...
        if (role) {
            // FIX: usar "colors" en lugar de "color" (deprecado)
            setInterval(() => role.edit({ colors: 'RANDOM' }).catch(() => {}), RAINBOW_MS);
            console.log('[ Rainbow ] Cambia 2 veces/seg ✅');
        } else {
            console.log(`[ Rainbow ] ⚠️ Rol ${ROLE_ID} no encontrado`);
        }
        ...
        try {
            await guild.commands.set([
                ...
                { name:'animal',     description:'🐾 Animal con dato en español' },
                ...
                { name:'hack',       description:'💻 Hackear (falso)', options:[{name:'usuario',type:6,description:'A quién',required:true}] },
                ...
                // Música
                { name:'play',   description:'▶️ Reproducir música', options:[{name:'cancion',type:3,description:'Nombre o URL de YouTube',required:true}] },
...
async function handleCommand(command, args, msg, inter) {
    ...
    const secondMention = () => Array.from(msg?.mentions?.users?.values() || [])[1] || null;
    ...
    if (command === 'play') {
        ...
        try {
            const result = await music.addSong(guild, vc, channel, query, author.username);
            const txt = result.position === 1
                ? `🔍 Buscando y reproduciendo: **${result.song.title}**...`
                : `✅ Agregado a la cola: **${result.song.title}** — posición #${result.position}`;
            if (inter) await inter.editReply(txt).catch(() => {});
            else channel.send(txt);
        } catch(e) {
            console.error('[ play ]', e.message);
            const errMsg = `❌ No pude reproducir eso.\n\`${String(e.message).slice(0,120)}\``;
            if (inter) { try { await inter.editReply(errMsg); } catch { inter.followUp(errMsg).catch(()=>{}); } }
    ...
    if (command === 'np') {
        const q = music.getQueue(guild);
        if (!q?.songs?.length) return reply('❌ No hay música reproduciéndose.');
        const s = q.songs[0];
        return reply(null, E('🎵 Sonando ahora', `**${s.title}** \`${s.duration}\`\nPedido por *${s.requestedBy}*`, '#7289DA'));
    }

    if (command === 'queue') {
        const q = music.getQueue(guild);
        if (!q?.songs?.length) return reply('📭 La cola está vacía.');
        const desc = q.songs.slice(0,15).map((s,i) =>
            `${i===0?'▶️ **Ahora:**':`**${i}.**`} ${s.title} \`${s.duration}\` — *${s.requestedBy}*`
        ).join('\n');
        return reply(null, new MessageEmbed().setTitle('🎵 Cola de música').setDescription(desc).setColor('#7289DA'));
    }

    if (command === 'skip') {
        try { music.skipSong(guild); return reply('⏭️ Canción saltada.'); }
        catch(e) { return reply(`❌ ${e.message}`); }
    }
    if (command === 'stop') {
        try { music.stopMusic(guild); return reply('⏹️ Música detenida.'); }
        catch(e) { return reply(`❌ ${e.message}`); }
    }
    if (command === 'pause') {
        try { music.pauseMusic(guild); return reply('⏸️ Pausada.'); }
        catch(e) { return reply(`❌ ${e.message}`); }
    }
    if (command === 'resume') {
        try { music.resumeMusic(guild); return reply('▶️ Reanudada.'); }
        catch(e) { return reply(`❌ ${e.message}`); }
    }
    if (command === 'volume') {
        const v = inter ? inter.options.getInteger('vol') : parseInt(args[0]);
        if (!v || v < 1 || v > 100) return reply('❌ Volumen entre 1 y 100.');
        try { music.setVolumeMusic(guild, v); return reply(`🔊 Volumen: **${v}%**`); }
        catch(e) { return reply(`❌ ${e.message}`); }
    }
    ...
    if (command === 'animal') {
        ...
        const embed = new MessageEmbed().setTitle(`${data.emoji} ${tipo.replace('_',' ').replace(/\b\w/g, c=>c.toUpperCase())}`).setDescription(`📖 **Dato curioso:** ${rnd(data.hechos)}`).setColor('#4CAF50');
        if (imgUrl) embed.setImage(imgUrl);
        return reply(null, embed);
    }

    if (command === 'dado') {
        const c = (inter ? inter.options.getInteger('caras') : parseInt(args[0])) || 6;
    ...
    if (command === 'love') {
        const t=mentionOrOpt('usuario'); if(!t) return reply('❌ Mencioná a alguien.');
    ...
    if (command === 'rate') {
        const cosa=inter?inter.options.getString('cosa'):args.join(' ').trim()||'eso', nota=randInt(0,10);
        return reply(null, E('⭐ Calificación',`**${cosa}**\n${'⭐'.repeat(nota)+'☆'.repeat(10-nota)}\n**${nota}/10**`,'#FFD700'));
    }

    if (command === 'ship') {
        const u1=inter?inter.options.getUser('user1'):msg?.mentions?.users?.first(), u2=inter?inter.options.getUser('user2'):secondMention();
        if(!u1||!u2) return reply('❌ Mencioná a dos personas.');
    ...
    if (command === 'pp') {
        const t=mentionOrOpt('usuario',author), sz=randInt(0,15);
        return reply(null, E('🍆 Medidor de PP',`**${t.username}**\n\n\`8${'='.repeat(sz)}D\`\n**${sz} cm**`,'#7CB342'));
    }

    if (command === 'iq') {
        const t=mentionOrOpt('usuario',author), iq=randInt(20,200);
    ...
    if (command === 'banana') {
        const t=mentionOrOpt('usuario',author), sz=randInt(0,20);
    ...
    if (command === 'hack') {
        ...
        const pasos=[`💻 Iniciando hack a **${t.username}**...`,'🔍 Escaneando puertos abiertos...','🔑 Crackeando contraseña...','📡 Bypaseando firewall...','🗄️ Accediendo a base de datos...',`✅ **${t.username}** hackeado! IP: \`192.168.${randInt(1,254)}.${randInt(1,254)}\` | Pass: \`hunter${randInt(1000,9999)}\` 💀`];
        const sent=await channel.send(pasos[0]).catch(()=>null); if(!sent) return;
        let i=0; const iv=setInterval(async()=>{ i++; if(i<pasos.length){try{await sent.edit(pasos[i]);}catch{}}else clearInterval(iv); },1200);
        return;
    }
    ...
    if (command === 'horoscopo') {
        const raw=(inter?inter.options.getString('signo'):args[0]||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim();
    ...
    // ── ECONOMÍA ──────────────────────────────────────────────────────────────
    if (command === 'perfil') {
        const t=mentionOrOpt('usuario',author), u=eco.getUser(t.id,t.username), need=eco.xpNeeded(u.level);
        return reply(null, new MessageEmbed().setTitle(`👤 Perfil de ${t.username}`).setThumbnail(t.displayAvatarURL({dynamic:true})).setColor('#FFD700')
            .addField('👛 Billetera',`${u.coins} 🪙`,true).addField('🏦 Banco',`${u.banco||0} 🪙`,true).addField('💎 Total',`${u.coins+(u.banco||0)} 🪙`,true)
            .addField('⭐ XP',`${u.xp}/${need}`,true).addField('🆙 Nivel',`${u.level}`,true).addField('📊 Progreso',barProgress(u.xp,need))
            .setFooter({text:'Ganás XP y coins chateando'}));
    }

    if (command === 'ranking') {
        const top=eco.getTop(10); if(!top.length) return reply('📭 No hay datos aún.');
        return reply(null, new MessageEmbed().setTitle('🏆 Top 10').setColor('#FFD700').setDescription(top.map((u,i)=>`${i===0?'🥇':i===1?'🥈':i===2?'🥉':`**${i+1}.**`} **${u.username}** — Nv.${u.level} | 💰${u.coins+(u.banco||0)}`).join('\n')));
    }
    ...
    if (command === 'work') {
        ...
        return reply(null, E('💼 ¡Trabajo completado!',`${r.job.verb}\n\n+**${r.earned} coins** 💰`,'#4CAF50').addField('👛 Billetera',`${r.total}`,true).setFooter({text:'Volvé en 1 hora'}));
    }

    if (command === 'slut') {
        const r=eco.doSlut(author.id,author.username);
        if(!r.success) return reply(`⏰ Volvé en **${fmtTime(r.remaining)}**.`);
        return reply(null, E(r.win?'💋 ¡Saliste bien!':'💋 No salió bien...',r.msg,r.win?'#FF69B4':'#E53935').addField(r.win?'💰 Ganaste':'💸 Perdiste',`${r.amount}`,true).addField('👛 Total',`${r.total}`,true).setFooter({text:'Volvé en 30 min'}));
    }

    if (command === 'crime') {
        const r=eco.doCrime(author.id,author.username);
        if(!r.success) return reply(`⏰ Volvé en **${fmtTime(r.remaining)}**.`);
        return reply(null, E(r.win?'🔫 ¡Crimen exitoso!':'🔫 ¡Te atraparon!',r.msg,r.win?'#B71C1C':'#E53935').addField(r.win?'💰 Ganaste':'💸 Perdiste',`${r.amount}`,true).addField('👛 Total',`${r.total}`,true).setFooter({text:'Volvé en 2 horas'}));
    }

    if (command === 'dep') {
        const amt=inter?inter.options.getString('cantidad'):args.join(' ').trim(), r=eco.deposit(author.id,author.username,amt);
    ...
    if (command === 'withdraw') {
        const amt=inter?inter.options.getString('cantidad'):args.join(' ').trim(), r=eco.withdraw(author.id,author.username,amt);
    ...
    if (command === 'rob') {
        ...
        return reply(null, E(r.win?'🥷 ¡Robo exitoso!':'🥷 ¡Te agarraron!',r.win?`Le robaste **${r.amount} coins** a **${r.targetName}** 😈`:`Intentaste robarle a **${r.targetName}** y te agarraron. Multa: **${r.fine} coins** 🚔`,r.win?'#FF6F00':'#E53935').addField('👛 Billetera',`${r.total}`,true).setFooter({text:'Volvé en 1 hora'}));
    }
}
