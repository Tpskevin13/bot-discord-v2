...
function doSpin(userId, username, bet) {
    ...
    const isRed=red.includes(n), isBlack=n>0&&!isRed, isZero=n===0;
    const color = isZero?'🟢 VERDE':isRed?'🔴 ROJO':'⚫ NEGRO';
    let mult = isZero ? 35 : 1; // verde paga 35x, rojo/negro paga 1x
    const win = !isZero;
    const earned = win ? bet*mult : -bet;
    u.coins = Math.max(0, u.coins+earned); u.lastSpin=now; u.username=username;
    if(earned>0) u.totalEarned=(u.totalEarned||0)+earned;
    data[userId]=u; save(data);
    return { success:true, n, color, win, earned, total:u.coins, bet };
}
